...
def plotRatioPlots(EADir, hdu, plotType, plate_IFU, Re, center, emLineInd, emLineFancy, nFP):
    dataInd = 1
    units = 'Some units'

    xValues, yValues, disValues, labelMat, mask, labels, counts = extractData(hdu, plotType, Re, emLineInd, dataInd)
...
#         print(labelVec[i] + ":  " + str(round(counts[i + 1] / counts[0] * 100, 2)) + '%')
    ...
    hex_at_Cen, gal_at_Cen = pT.getCenters(hdu, plate_IFU, dataInd)
    ...
    pF.opticalImage(EADir, hdu, plate_IFU, Re, center, dataInd, axes1)

    axes2 = plt.subplot(1, 3, 2)
    ratioAxes(plotType, emLineFancy, xValues, yValues, disValues, labels, axes2)

    axes3 = plt.subplot(1, 3, 3)
    pF.spatiallyResolvedPlot(hdu, plotType, plotType, plate_IFU, Re, center, dataInd, units, labelMat, mask, hex_at_Cen, gal_at_Cen, vmax, vmin, axes3)
    dOP.addReCircles(axes3)
    fig.tight_layout()
    # plt.show()
    # print(jello)
    plt.savefig(nFP + plate_IFU + '_' + plotType + '.png', bbox_inches='tight')
    # print(jello)
    plt.close()
